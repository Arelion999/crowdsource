...
Всего батчей: **996** | строк: **472,033** | заполнено: **456,158** (96.6%), пусто **15,875** | вычитано человеком: **95,397** (20.2%).
...
| `misc_007.csv` | misc | 500 | 24,393 | Choose between searching for the prisoners an | ✅ | Arelion999 — вычитан: 3 ошибки переносов в письмах NPC, подменённый машиной перевод главы истории, имена из слоя с падежом, обращение к игроку сведено к «вы» (135 строк) |
...
| `ui_116.csv` | ui | 496 | 16,121 | Recipe[s]: Chiaroscuro Hammers | ✅ | Arelion999 — вычитан: 202 склейки раскрыты, регистр после двоеточия к канону new_023, прямые кавычки к ёлочкам, коллизия Pauldrons/Shoulderguard разведена (349 строк) |
| `ui_117.csv` | ui | 500 | 15,889 | Recipe[s]: Gift of Music | ✅ | Arelion999 — вычитан: 200 склеек раскрыты, регистр после двоеточия к канону new_023 (строчная), коллизии типов вещей разведены (335 строк) |
| `ui_118.csv` | ui | 499 | 15,713 | Recipe[s]: Lightborne Shield | ✅ | Arelion999 — вычитан: 189 склеек раскрыты, регистр после двоеточия к канону new_023, Lightborne → «светорождённый» по ui_008, девять коллизий типов вещей разведены (395 строк) |
| `ui_119.csv` | ui | 500 | 15,708 | Recipe[s]: Raven Totem | ✅ | Arelion999 — вычитан: 180 склеек «Рецепт[|а|ов]» раскрыты полными формами по README, четыре коллизии типов вещей разведены по items_013/014/018, подменённый перевод Recycler исправлен, регистр после двоеточия доведён до канона new_023 (182 строк) |
| `ui_120.csv` | ui | 498 | 14,708 | Recipe[s]: Stormcaller Greatsword | ✅ | Arelion999 — вычитан: 246 склеек раскрыты, регистр после двоеточия к канону new_023, Stormcaller → «Зов бури» и Greatsword → «двуручный меч» по items_006/013, четыре коллизии типов вещей разведены (371 строк) |
...
| `ach_005.csv` | ach | 500 | 27,500 | Chasing Tales: Warrior Poet | ✅ | Arelion999 — вычитан: рамка Chasing Tales к канону корпуса, обращение к игроку на «вы», имена и топонимы из слоя с падежом, коллизия пяти наборов разведена (119 строк) |
...
| `ach_007.csv` | ach | 500 | 33,103 | Complete all of the "Clear out the elementals | ✅ | Arelion999 — вычитан: топонимы из слоя с падежом (таблица 132 имени), разведены Dragon’s End и Dragonfall, снят знак ударения, 2 пустые ячейки заполнены (242 строк) |
...
| `ach_010.csv` | ach | 500 | 15,008 | Daily Maguuma Jungle Lumberer | ✅ | Arelion999 — вычитан: рамки ежедневок к канону (прыжковая головоломка, мини-подземелье, реагирование на драконов, завершение событий), топонимы из слоя, род прилагательного по месту (122 строки) |
...
| `ach_012.csv` | ach | 500 | 25,141 | Defeat the grand champion in the Ooze Pit. | ✅ | Arelion999 — вычитан: топонимы из слоя с падежом (109 имён в таблице), Defeat → «Победите» по корпусу, Icebrood → «ледяное отродье», группы форм полными словами, 3 пустые ячейки заполнены (168 строк) |
| `ach_013.csv` | ach | 500 | 24,110 | Do not trigger any Gravitational Waves during | ✅ | Arelion999 — вычитан: имена и топонимы из слоя с падежом по контексту, три формы при счётчике, прямые кавычки к ёлочкам, 4 пустые ячейки заполнены (134 строки) |
...
| `ach_020.csv` | ach | 500 | 16,863 | League Conqueror—Conquest Season Forty-Four | ✅ | Arelion999 — вычитан: имена и топонимы из слоя, падежи имён с титулами, полуперевод откачен (110 строк) |
...
| `ach_025.csv` | ach | 500 | 17,328 | Repel the Branded from the Free City of Amnoo | ✅ | Arelion999 — вычитан: топонимы из слоя с падежом, «Львиная Аркада» → «Львиная Арка», формы при счётчике, пустая ячейка; названия эпизодов в рамке Return to оставлены латиницей по канону new_024 (117 строк) |
