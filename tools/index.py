...
Реестр названий (какие строки — названия и в каком словаре pn_* им место)
строится этим же прогоном и спрашивается через tools/names.py.
...
def cmd_build(_args):
    ...
    # Реестр названий: что является названием, в каком словаре pn_* ему место и
    # в каком файле батча. Держим в той же базе и строим тем же прогоном — иначе
    # реестр разъедется с графом, а разъезда никто не увидит.
    db.close()
    try:
        import names as N
        print()
        ...
        print("  реестр названий не построился: %s" % e)
