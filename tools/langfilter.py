...
# Умлаут, ß, ñ, перевёрнутые знаки — в английском клиенте их не бывает почти
# никогда, и один такой знак уже улика. Прочие правила требуют рядом служебное
# слово языка либо второе слово со знаком, поэтому одиночный композит
# («Kräutersprosse», «Winterwurzelgemüse», «Kampfreißer») проходил мимо всех.
DIA_HARD = re.compile(r"[äöüßÄÖÜñÑ¡¿]")
# Исключения — заимствования, которые английский клиент пишет со знаком сам.
# Главное здесь — семейство оружия Einhänder/Zweihänder: это английские названия,
# видно по английским аффиксам вокруг («Healing Einhänder of Agony»), немецкий
# клиент отдал бы всю строку по-немецки. Их 32 из 39 всех находок признака.
DIA_LOAN = re.compile(r"Einhänder|Zweihänder|Español|Mjölnir|Jökulhlaup|piñata|jalapeño"
                      r"|El Niño|señor|Doppelgänger|Über|Fräulein|naïve|café|éclair"
                      r"|façade|jalapeno", re.I)
...
def verdict(en_text, ru_text=""):
    ...
    # Умлаут, ß или ñ сам по себе — почти доказательство, но «почти»: в корпусе
    # есть заимствования, которые английский клиент пишет так же (семейство
    # Einhänder/Zweihänder — 32 записи). Поэтому «под вопросом», а не «уверенно»:
    # `apply` такие не трогает, зато они наконец попадают в отчёт. Этот признак
    # не требует ни служебного слова рядом, ни второго знака, и тем отличается от
    # правил выше — одиночный композит виден только ему.
    # Гейта по английским словам тут нет намеренно: «die», «war», «hat», «will»
    # лежат в обоих списках, и немецкая фраза набирает английское слово на пустом
    # месте («Lest die Titanen-Geschichtsbücher in Sammlersruh»). Замер: по всему
    # корпусу знак вне аллошиста несут ровно шесть строк, так что гейт не убирал
    # шум, а прятал одну из шести.
    if DIA_HARD.search(e) and not DIA_LOAN.search(e):
        lg = "de" if DIA_DE.search(e) else "es" if DIA_ES.search(e) else lang
        return lg, "под вопросом", "умлаут/ß/ñ в оригинале — английский клиент так не пишет"
...
def cmd_markers(args):
    ...
    shas = []
    # Список пополняется вместе с формулировками коммитов: чистка немецкого и
    # испанского подписана «мёртвых записей: english не английский», и по двум
    # исходным образцам она не находилась вовсе — 236 снятых строк в словарь
    # не попадали, а именно они и учат его немецкому.
    for pat in ("французск", "не-английск", "не английск", "мёртвых записей"):
